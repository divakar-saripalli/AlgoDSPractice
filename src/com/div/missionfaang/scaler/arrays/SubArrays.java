    ...
    private static int subarrayWithMaximumSumWithinB(ArrayList<Integer> C, int B, int A) {
        ...
        for (int i = 0; i < A; i++) {
            for (int j = i; j < A; j++) {
                int currentAverage;
                if (i == 0) {
                    currentAverage = prefixSumArray.get(j);
                } else {
                    currentAverage = (prefixSumArray.get(j - 1) - prefixSumArray.get(i));
                }
                if (currentAverage > maxSumWithinB && currentAverage <= B) {
                    maxSumWithinB = currentAverage;
                }
            }
        }
